...
# Function to check if removing one number makes sequence safe
def single_bad_level_error_safe(line: List[int]) -> bool:
    # Try removing each number one at a time
    for i in range(len(line)):
        line_without_error = line.copy()
        line_without_error.pop(i)
        # Check if sequence is safe with number removed
        if calculate_safety_rating(line_without_error):
            return True
    return False
...
def calculate_safety_rating_sum(data: str) -> int:
    ...
    for line in data.splitlines():
        ...
        if calculate_safety_rating(line):
            sum += 1
        # Check if sequence can be made safe by removing one number
        elif single_bad_level_error_safe(line):
            sum += 1
    return sum
